...
import { saveEmployee, updateEmployee, deleteEmployee } from '../lib/db';
...
export default function Employees() {
    ...
    function activeOrderCount(empId: string, role: EmployeeRole) {
        return orders.filter(o =>
            ...
            (role === 'admin' || role === 'lead_florist' || role === 'florist' ? o.floristId === empId : o.shipperId === empId)
        ).length;
    }
    ...
    async function handleSave() {
        ...
        if (editTarget) {
            await updateEmployee({ ...editTarget, name: form.name.trim(), username: form.username.trim(), password: form.password.trim(), role: form.role });
            toast.success('Cập nhật nhân viên thành công');
        } else {
            await saveEmployee({ id: genEmpId(), name: form.name.trim(), username: form.username.trim(), password: form.password.trim(), role: form.role, isAdmin: false });
        ...
        return (
            <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
                <div className="flex items-start justify-between">
                    <div>
                        ...
                        <span className={clsx(
                            ...
                            emp.role === 'admin' ? 'bg-gray-100 text-gray-700' :
                ...
                <div className="mt-3 pt-3 border-t border-gray-100 grid grid-cols-2 gap-2 text-xs text-gray-500">
                    <div>
                        <span className="text-gray-400">{emp.role === 'admin' || emp.role === 'florist' || emp.role === 'lead_florist' ? 'Đang làm:' : 'Đang giao:'} </span>
    ...
    if (empsLoading || ordersLoading) return (
        <div className="flex items-center justify-center min-h-[400px]">
            <div className="text-gray-400 animate-pulse">Đang tải nhân sự...</div>
        </div>
    );
            ...
            {admins.length > 0 && (
                <div>
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <h2 className="font-semibold text-gray-700">Quản Trị Viên ({admins.length})</h2>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {admins.map(e => <EmpCard key={e.id} emp={e} />)}
                    </div>
                </div>
            )}
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    {(['admin', 'lead_florist', 'florist', 'dispatcher'] as EmployeeRole[]).map(r => (
                                        <button
                                            key={r}
                                            ...
                                            className={clsx(
                                                ...
                                                form.role === r
                                                    ? r === 'admin' ? 'border-gray-900 bg-gray-50 text-gray-900' :
                                        ...
                                        >
                                            {r === 'admin' ? <Users size={14} /> : r === 'dispatcher' ? <Truck size={14} /> : <Flower2 size={14} />}
                                            {r === 'admin' ? 'Quản trị' : r === 'lead_florist' ? 'Thợ chính' : r === 'florist' ? 'Thợ cắm hoa' : 'Vận đơn'}
                                        </button>
                                    ))}
                                </div>
                            </div>
